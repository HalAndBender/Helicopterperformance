...
def operation_result():
    ...
    try:
        ...
        d.text((5840,3250),str(QNH) + ' hPa' ,(0,0,255),font=font)
...
def AW139_dropdown_6800_result():
    ...
    try:
        ...
        d.text((vertical_align,2750+horizontal_align),str(QNH) + ' hPa' ,(0,0,255),font=font)
...
def AW169_OGE_OEI_result():
    ...
    # DOM = 3000
    # QNH = 1013.2                # mb (min 971, max 1044)
    # height = 1000          # feet (min 0, max 350)
    # temp = 10                   # degrees celcius (min: -10, max: 30)
    # wind = 26               # knots (min: 0, max: 40)
    # perf_benefit = 100          # percent
    # fuel_at_hho = 439           # kg
    ...
    lower_PA_pixel = 3312 # here: 0ft
    upper_PA_pixel = 1329  # here: 15000ft
    diff_upper_lower_PA = 15000 # difference between upper and lower PA on chart
    ...
    # start point on vertical-axis
    input_vertical_pixel = 580 # this is an x-value
    point_1 = (input_vertical_pixel, PA_pixel )              # defining starting point on chart
    d.text((input_vertical_pixel,PA_pixel-80 ),str(result_PA) + ' ft' , fontcolor,font=font) # labeling the point

    '''left part of manufacturers chart'''
    # pixel values on x- and y-axis
    x_3=[ 822,1216,1611,1942] #-20
    y_3=[1829,2113,2386,2599]
    x_4=[ 822,1216,1611,1951] #-10
    y_4=[1972,2262,2537,2755]
    x_5=[ 822,1216,1611,1963] # 0
    y_5=[2140,2429,2703,2928]
    x_6=[ 822,1216,1611,1974] # +10
    y_6=[2300,2594,2866,3099]
    x_7=[ 822,1216,1611,1990] # +20
    y_7=[2487,2780,3057,3303]
    x_8=[ 822,1216,1611,1931] # +30
    y_8=[2654,2951,3231,3445]
    x_9=[ 822,1216,1611] # +40
    y_9=[2856,3152,3434]
    x_10=[ 822,1138,1289] # +50
    y_10=[3106,3327,3445]

    # dictionary for lines
    dictionary ={-20:[x_3,y_3],
                 -10:[x_4,y_4],
                 0:[x_5,y_5],
                 10:[x_6,y_6],
                 20:[x_7,y_7],
    ...
    point_3 = (PA_temp_pixel, 3587)

    first_chart_pixel = input_pixel
    ...
    # chart_lower_value = 821 # choose x- or y- values
    # chart_upper_value = 2000  # choose x- or y- values
    ...
    '''right part of manufacturers chart'''
    #the separation line
    input_pixel = output_pixel
    chart_lower_value = 1943 # choose x- or y- values
    chart_upper_value = 2020  # choose x- or y- values
    x_s=[1943,1990]
    y_s=[2599,3300]
    point_list = []
    model = np.poly1d(np.polyfit(x_s,y_s, len(x_s)-1))
    ...
    output_pixel = model(input_pixel)


    if output_pixel > first_chart_pixel:
        # pixel values on x- and y-axis
        x_3=[1943,2068,2157] #-20
        y_3=[2599,3050,3444]
        x_4=[1951,2049,2127] #-10
        y_4=[2760,3115,3445]
        x_5=[1962,2031,2096] # 0
        y_5=[2931,3181,3446]
        x_6=[1974,2022,2065 ] # +10
        y_6=[3102,3279,3445]
        x_7=[1992,2030] # +20
        y_7=[3301,3445]
        x_8=[ 822,1216,1611,1931] # +30
        ...
        # input_is_x = 0 # set to 1 if input is x
        # output_is_y = 1 # set to 0 if output is y
        # chart_lower_value = 1940 # choose x- or y- values
        # chart_upper_value = 2188  # choose x- or y- values
        # for key in dictionary.keys():
        #     x_and_y = dictionary.get(key)
        #     model = np.poly1d(np.polyfit(x_and_y[input_is_x], x_and_y[output_is_y], len(x_and_y[0])-1))
        #     point_list = []
    ...
    # convert pixel output to chart value
    lower_pixel = 822   # here: 3300kg
    upper_pixel = 2926  # here: 4900kg
    diff_value_kg =  4900 - 3300
    diff_value_pixel = upper_pixel  - lower_pixel

    pixel_per_kg = diff_value_pixel / diff_value_kg

    zero_wind_mass = 3300 + ( PA_temp_pixel - lower_pixel )/pixel_per_kg
    ...
    #d.text((60, PA_pixel-80),str(result_PA) + ' ft' , fontcolor,font=font)
    d.text((PA_temp_pixel, PA_pixel-80),str(temp) + ' C' ,(0,0,255),font=font)
    d.text((PA_temp_pixel+10, 3530),str(result_zero_wind_mass) + ' kg' , fontcolor,font=font)

    # wind box
    begin = 1329  # zero knots
    end = 3093    # 50 knots
    fieldlength = (end - begin)
    pix_per_knot = fieldlength / 50
    zeroknots = begin
    windpix = zeroknots + (pix_per_knot * wind)
    top = 3744
    bottom = 4110
...
def AW169_dropdown_4200_result():
    ...
    try:
        ...
        # d.text((vertical_align,2050+horizontal_align),'USER INPUT: ',(0,0,0),font=font)
        d.text((vertical_align,2400+horizontal_align),"QNH:" ,(0,0,0),font=font)
        d.text((vertical_align,2450+horizontal_align),str(QNH) + ' hPa' ,(0,0,255),font=font)
        d.text((vertical_align,2600+horizontal_align),"Hover Height:" ,(0,0,0),font=font)
...
def AW139_rejected_tod_clear_area_result():
    ...
    d.text((vertical_align,2300+horizontal_align),str(QNH) + ' hPa' ,(0,0,255),font=font)
